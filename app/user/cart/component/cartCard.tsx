   ...
   return (
     <main key={cartItem.product.id}>
       ...
       <div className=" items-center gap-5 py-3 px-6 flex justify-between ">
         <button className="flex gap-2" onClick={removeItem}>
           <Trash className="text-orange-700" />
           Remove
         </button>

         <div className="flex items-center border rounded">
           <button className="px-3 py-1" onClick={decreaseAmount}>
             -
           </button>
           <span className="px-4">{cartItem.amount}</span>
           <button className="px-3 py-1" onClick={increaseAmount}>
             +
           </button>
         </div>
       </div>
